...
function normalizeTags(tags: string[] | undefined, categoryLabel: string, allowMultipleTags: boolean) {
...
export function createBlog(config: BlogCreateConfig) {
	...
	async function getAllAuthors(): Promise<BlogAuthor[]> {
		const posts = await getAllPosts();
		const map = new Map<string, BlogAuthor>();
		for (const post of posts) map.set(post.author.id, post.author);
		return Array.from(map.values()).sort((a, b) => a.name.localeCompare(b.name));
	}

	async function getAuthorSummaries(): Promise<BlogAuthorSummary[]> {
		const posts = await getAllPosts();
	...
	async function getArchiveGroups(): Promise<BlogArchiveGroup[]> {
		const posts = await getAllPosts();
		const map = new Map<string, BlogArchiveGroup>();
		for (const post of posts) {
	...
	async function getRelatedPosts(slug: string, limit = 3): Promise<BlogPost[]> {
		const posts = await getAllPosts();
		...
		if (primaryTag) {
			...
			const sameTag = posts
				.filter(
					(post) =>
						post.slug !== slug && post.tags.some((tagName) => slugify(tagName) === primaryTagSlug)
				)
				.sort((a, b) => parseISODate(b.date).getTime() - parseISODate(a.date).getTime())
				.slice(0, limit);
			if (sameTag.length > 0) return sameTag;
		}
